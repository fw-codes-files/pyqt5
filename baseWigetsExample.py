...
def baseWigets():
    app = QApplication(sys.argv)  # 每个Gui都必有，参数用来获取命令行参数，不用命令行可以[]替代
    win = QMainWindow()  # 画布
    win.setGeometry(400, 400, 400, 300)  # 定义画布尺寸，xywh
    win.setWindowTitle("Pyqt5 Tutorial")

    # Label Text
    label = QLabel(win)
    label.resize(200, 100)  # 整个label大小，类似margin
    label.setText("Hi this is Pyqt5")
    label.move(100, 100)  # 左上角位置
    ...
    win.show()
    sys.exit(app.exec_())  # 设置窗口一直运行直到使用关闭按钮进行关闭
